# Nhật Ký Học Tập — MangaVault

> File này ghi lại quá trình học Spring Boot theo từng task nhỏ, từ lúc project còn là app CRUD manga cá nhân. Từ Giai đoạn 1 trở đi (cuối file), project đã pivot thành **website đọc truyện trực tuyến** phục vụ đồ án thực tập — xem `README.md` cho hiện trạng và `docs/HƯỚNG ĐI THỰC TẬP.md` cho yêu cầu chính thức. Các task 1 → 3.10 bên dưới được giữ nguyên làm tư liệu học tập (domain lúc đó còn là `Manga`, nay đã đổi thành `Story`).
...
# Từ đây trở đi: pivot sang đồ án thực tập

Sau Task 3.10, project nhận tài liệu yêu cầu chính thức (`docs/HƯỚNG ĐI THỰC TẬP.md`) và pivot từ "app quản lý manga cá nhân" thành **website đọc truyện trực tuyến** (React + Spring Boot). Cách ghi chép cũng đổi: thay vì từng task nhỏ, mỗi giai đoạn được tổng kết lại kèm bài học.

## Giai Đoạn 1: Pivot Domain Manga → Story/Chapter + Frontend React

### Những gì đã làm

- Rename entity `Manga` → `Story` (bảng `stories`), thêm field `author`.
- Entity mới `Chapter`: `@ManyToOne(fetch = LAZY)` về `Story`, `chapterNumber` với unique constraint `(story_id, chapter_number)`, `content` kiểu `LONGTEXT`.
- API đọc công khai: danh sách (tìm kiếm + phân trang qua `PageResponse<T>`), chi tiết truyện, danh sách chương (không bao giờ trả `content`), đọc chương (kèm `prevChapterId`/`nextChapterId`).
- Frontend React (Vite) hoàn chỉnh: danh sách + tìm kiếm + phân trang, chi tiết truyện, trang đọc chương với nút chương trước/sau. API call tập trung tại `src/services/`.
- Hạ tầng: MySQL 8.4 chạy bằng `docker-compose.yml`, CORS config cho dev server Vite.

### Bài học chính

- **Phân trang với Spring Data**: `Pageable` + `Page<T>` làm gần hết việc; DTO `PageResponse<T>` với method `from(Page<E>, Function<E,T>)` giúp mọi API danh sách trả cùng một cấu trúc.
- **Tách summary/detail DTO có chủ đích**: danh sách chương chỉ trả `ChapterSummaryResponse` (không có `content`) — tránh tải cả bộ truyện khi chỉ cần mục lục.
- **Prev/next không được giả định số liên tục**: dùng `findFirst...LessThan/GreaterThan` (nearest-neighbor) thay vì `chapterNumber ± 1`, để điều hướng vẫn đúng khi có chương bị xóa.
- **Lỗi phải trả đúng envelope kể cả khi JSON hỏng**: thêm handler `HttpMessageNotReadableException`, nếu không Spring trả error body mặc định làm frontend vỡ format.
- **Encoding tiếng Việt trên Windows**: seed dữ liệu qua `curl -d` inline bị hỏng UTF-8; phải ghi JSON ra file rồi `--data-binary @file`.

## Giai Đoạn 2: Admin CRUD Truyện + Chương

### Những gì đã làm

- `ErrorCode` enum: mỗi lỗi nghiệp vụ mang cả HTTP status lẫn mã ứng dụng (`4041`/`4042` not found, `4090` trùng slug, `4091` trùng số chương). `AppException` nhận `ErrorCode` thay vì String.
- `slug` unique: constraint ở DB + check `existsBySlug`/`existsBySlugAndIdNot` ở service để trả 409 có thông điệp rõ ràng.
- API admin dưới `/api/admin/**`: tạo/sửa/xóa truyện, tạo/sửa/xóa chương. Xóa truyện xóa luôn chương trong một `@Transactional`.
- Trang quản trị React: bảng truyện (sửa/xóa), form tạo/sửa dùng chung một component, trang quản lý chương với form thêm/sửa inline và tự gợi ý số chương kế tiếp.

### Bài học chính

- **Enum ErrorCode giải quyết "mọi lỗi đều 404"**: khi exception mang sẵn status + code, `GlobalExceptionHandler` chỉ còn một handler đọc từ enum — thêm loại lỗi mới không phải sửa handler. Đây là điều kiện tiên quyết trước khi làm auth (cần 401/403).
- **Đặt URL theo ranh giới phân quyền ngay từ đầu**: mọi endpoint ghi nằm dưới `/api/admin/**` dù chưa có security — sang giai đoạn auth chỉ cần một rule theo path, không phải di chuyển endpoint.
- **Unique check hai tầng**: constraint DB là chốt chặn cuối (chống race condition), nhưng check ở service mới cho phép trả 409 + thông điệp thân thiện thay vì lộ lỗi SQL.
- **Cascade delete phải chủ động**: `Chapter` giữ FK nên xóa `Story` trần sẽ vỡ constraint; service phải xóa chương trước trong cùng transaction (hoặc khai báo cascade ở mapping một cách có chủ đích).
